...
import { updateTeam } from "../firebase/updateFunctions";
...
const EditTeam = ({ navigation, route}) => {
  ...
  const { teamId, tournamentId, createdBy, teamName, teamImage } = route?.params
  console.log(teamId)
  const [imagen, setImagen] = useState(teamImage);
  ...
  async function handleSubmit({ nombre, img }) {
    if (nombre) {
      ...
      const resp = await updateTeam( createdBy, tournamentId, teamId, data);

      if (resp) {
        Toast.show({
          type: "success",
          text1: "😃 Editado con Exito",
        });
  ...
  return (
    <>
      ...
      >
        <Formik
          initialValues={{ nombre: teamName, img: teamImage }}
